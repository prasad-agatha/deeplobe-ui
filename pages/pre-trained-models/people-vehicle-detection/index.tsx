...
import { peopleVehicleSampleImages } from "common_functions/sample_images.ts/peopleVehicleSampleImages";
// toasts
...
const peopleVehicleDetection: FC = () => {
  ...
  const [test_array, setTest_array] = useState(peopleVehicleSampleImages);
  ...
  const detectFaces = async () => {
    ...
    if (Object.keys(uploadedFile).length > 0 || imageUrl.length > 0 || selectedImages.length > 0) {
      ...
      if (error) {
        ...
        tempfile = formData;
        pretrainedService
          .peoplevehicledetection(tempfile)
          .then((res) => {
            setResult({
              ...result,
              original_image_url: res.original_image_url,
              image_url: res.annotated_image_url,
              no_of_people: res.no_of_people,
              no_of_vehicles: res.no_of_vehicles,
            });
            setLoading(false);
          })
          .catch((err) => {
  ...
  const resetAll = (e) => {
    ...
    setTest_array(peopleVehicleSampleImages);
    setSelectedImages([]);
  };
  ...
  const imageClick = (image) => {
    ...
    //Reset Upload images
    setSelectedImages([]);
    let updated_sample = peopleVehicleSampleImages.map((item: any) => {
      if (item.id == image.id) {
        return {
          id: image.id,
        ...
        <div className="">
          <div className="row mx-5 my-5">
            ...
            <div className="card border-0 p-3 bg-white">
              <div className="">
                <h4 className="page-title ms-3">People and vehicle detection</h4>
              </div>
            </div>
            ...
            <div className="card border-0 bg-white pb-3">
              <div className="my-2 mx-4 page-content">
                <p className="mb-3">
                  Identify people and vehicles within an image. The model detects and classifies an
                  image into two classes - people and vehicles, and provides a probability score
                  emphasizing the objects with bounding boxes for these classes. Test the model by
              ...
              {submit ? (
                <>
                  <div className="container">
                    ...
                    <>
                      {loading ? (
                        <ShimmerThumbnail className="mt-1" height={30} width={300} rounded />
                      ) : (
                        <h5 className="text-left result-content">
                          Number of vehicles detected: {result.no_of_vehicles}
                        </h5>
                      )}
                      {loading ? (
                        <ShimmerThumbnail className="mt-0" height={30} width={300} rounded />
                      ) : (
                        <h5 className="text-left result-content">
                          Number of people detected: {result.no_of_people}
                        </h5>
                      )}
                    </>
                  </div>
...
export default peopleVehicleDetection;
