...
import { panOcrSampleImages } from "common_functions/sample_images.ts/ocrSampleImages";
// toasts
...
const panOCR: FC = () => {
  ...
  const [test_array, setTest_array] = useState(panOcrSampleImages);
  ...
  const resetAll = (e) => {
    ...
    setTest_array(panOcrSampleImages);
    setSelectedImages([]);
  };
  ...
  const imageClick = (image) => {
    ...
    //Reset Upload images
    setSelectedImages([]);
    let updated_sample = panOcrSampleImages.map((item: any) => {
      if (item.id == image.id) {
        return {
          id: image.id,
        ...
        <div className="">
          <div className="row mx-5 my-5">
            ...
            <div className="card border-0 p-3 bg-white">
              <div className="">
                <h4 className="page-title ms-3">PAN data extractor</h4>
              </div>
            </div>
            ...
            <div className="card border-0 bg-white pb-3">
              <div className="my-2 mx-4 page-content">
                <p className="mb-3">
                  Automatically identify and extract PAN data like full name, father name, date of
                  birth(DOB), permanent account number(PAN) from the given image.
                </p>
              </div>
              <SampleCodeAccordion
                isPretrained={true}
                isImageInput={true}
                url1={`${process.env.NEXT_PUBLIC_API_SERVER}/api/pretrained-models/pan_data_extractor`}
                url2={``}
                url3={``}
