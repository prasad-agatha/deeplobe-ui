...
import { imageSearchSimilaritySampleImages } from "common_functions/sample_images.ts/imageSimilaritySampleImages";
// toasts
...
const imageSimilarity: FC = () => {
  ...
  const [test_array, setTest_array] = useState(imageSearchSimilaritySampleImages);
  const [img_Arr, setImgArr] = useState([] as any);
  const [noOfsimilarImages, setNoOfsimilarImages]: any = useState(1);
  ...
  const detectFaces = async () => {
    ...
    if (
      ...
      if (selectedUploadType === "upload-image") {
        ...
        if (imageExists1 && imageExists2) {
          try {
            ...
            formData.append("k_neighbours", noOfsimilarImages);
        ...
        const formData = new FormData();
        let base64Image: any = await getDataBlob(uploadedFile[0].original_path);
      ...
      if (error) {
        ...
        pl = imgArr;
        const formdata = new FormData();
        formdata.append("name[]", actual_file.name);
        formdata.append("asset[]", actual_file);
        trainService
          .uploadAsset(formdata)
          .then((res) => {
            const payload = {
              input_url: res[0].asset,
              k_neighbours: parseInt(noOfsimilarImages),
            };
            trainService;
            pretrainedService
              .imageSearchSimilarity(payload)
              .then((res) => {
                if (res === undefined) {
                  resetAll();
  ...
  const resetAll = () => {
    setNoOfsimilarImages(1);
    setResult({} as any);
    setSubmit(false);
    setUploadedFile([]);
    setSelectedUploadType("upload-image");
    setTest_array(imageSearchSimilaritySampleImages);
    setSelectedImage([]);
    setImgArr([]);
    setSelectedImages([]);
  };
  ...
  const imageClick = (image) => {
    ...
    setSelectedImages([]);
    img_Arr.push(image);
    if ([...selectedImage, image.id].length > 1) {
      selectedImage.shift();
      img_Arr.shift();
    ...
    for (let item of imageSearchSimilaritySampleImages) {
        ...
        <div className="">
          <div className="row mx-5 mt-5">
            ...
            <div className="card border-0 p-3 bg-white">
              <div className="">
                <h4 className="page-title ms-3">Image Similarity Search</h4>
              </div>
            </div>
            ...
            <div className="card border-0 bg-white pb-3">
              <div className="my-2 mx-4 page-content">
                <p className="mb-3">
                  Effortlessly explore visually similar images from your database in real-time. Test
                  our powerful image similarity search model by choosing from a variety of provided
                  sample images or by uploading your own. Specify the desired number of similar
                  images to retrieve from the database and initiate the search by simply clicking
                  "Run model". Please note that in this example, we utilized a jewelry image
                  database to demonstrate the capabilities of our image similarity search model.
                  When testing with your own images, kindly ensure they are related to jewelry.
                </p>
              </div>
              <SampleCodeAccordion
                isPretrained={true}
              ...
              <div className="m-3 row">
                <div className="col-lg-6 p-0">
                  <span>Select the no. of similar images do you need</span>
                  <select
                    className=" font-inter mx-2 ps-1"
                    id="inputGroupSelect03"
                    style={{ backgroundColor: "#EDF2F7", width: "45px" }}
                    value={noOfsimilarImages}
                    onChange={(e) => {
                      setNoOfsimilarImages(e.target.value);
                    }}
                    required
                  >
                    <option value="" hidden>
                    ...
                    {loading ? (
                      ...
                      <div>
                        <Tabs
                          ...
                          className="border-0"
                        >
                          <Tab eventKey="Labels" title="Similar images" className="p-3">
                            <div className="row justify-content-around mt-3 mb-2 gap-3 gap-xl-0">
                              <div className="col-12 d-flex flex-column">
                                <div className="text-left">
                                  <h6 className="mt-2">Input image</h6>
                                </div>
                            ...
                            <div className="row justify-content-around mt-3 mb-2 gap-3 gap-xl-0">
                              <h6 className="mt-2">Similar images</h6>
                              {result?.similar_images.map((ele, id) => (
                                //   <div>
                                <div className="col-12 col-xl-6 d-flex flex-column">
                                  ...
                                  <h5 className="d-flex flex-row  result-content">
                                    Similarity score: {parseFloat(ele.similarity_score).toFixed(2)}{" "}
                                    out 100
                                  </h5>
                                </div>
                                //   </div>
                          ...
                          <Tab eventKey="JSON" title="JSON" className="p-3">
                            <div style={{ position: "relative" }}>
                              ...
                              >
                                <button
                                  ...
                                  onClick={() => copy(JSON.stringify(result))}
                                >
                            ...
                            <JsonFormatter json={JSON.stringify(result)} />
                          </Tab>
                        </Tabs>
                      </div>
                    )}
