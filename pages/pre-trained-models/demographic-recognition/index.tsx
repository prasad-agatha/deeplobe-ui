...
import { demographicSampleImages } from "common_functions/sample_images.ts/demographicSampleImages";
// toasts
...
const demographicDetection: FC = () => {
  ...
  const [test_array, setTest_array] = useState(demographicSampleImages);
  ...
  const detectFaces = async () => {
    ...
    if (Object.keys(uploadedFile).length > 0 || imageUrl.length > 0 || selectedImages.length > 0) {
      ...
      if (error) {
        ...
        tempfile = formData;
        pretrainedService
          .demographicRecognition(tempfile)
          .then((res) => {
            setResult({
              ...result,
  ...
  const resetAll = (e) => {
    ...
    setTest_array(demographicSampleImages);
    setSelectedImages([]);
  };
  ...
  const imageClick = (image) => {
    ...
    //Reset Upload images
    setSelectedImages([]);
    let updated_sample = demographicSampleImages.map((item: any) => {
      if (item.id == image.id) {
        return {
          id: image.id,
        ...
        <div className="">
          <div className="row mx-5 my-5">
            ...
            <div className="card border-0 p-3 bg-white">
              <div className="">
                <h4 className="page-title ms-3">Demographic recognition</h4>
              </div>
            </div>
            ...
            <div className="card border-0 bg-white pb-3">
              <div className="my-2 mx-4 page-content">
                <p className="mb-3">
                  Predict and label the age and gender of people within an image. To test your
                  model’s accuracy, you can select any image from the sample images provided below
                  or, alternatively, you can either upload an image from your computer or supply an
                  URL of any image in the box provided below. Click “Run model”. Our model will
                  provide you with the most important demographic details of anyone present in the
                  image.
                </p>
                <p className="mb-0">Disclaimer: This model works only for adults above 20 years</p>
              </div>
              <SampleCodeAccordion
                isPretrained={true}
                isImageInput={true}
                ...
                url2={``}
                url3={``}
                modelName={"demographic-recognition"}
                ...
                <div className="col-md-5  block-background container p-2">
                  <div className="d-flex flex-wrap gap-3">
                    <div className="form-check">
                      <input
                        className="form-check-input cr-p"
                        type="radio"
                      ...
                      />
                      <label
                        className="form-check-label page-content cr-p"
                        onClick={() => {
                          setSelectedUploadType("upload-image");
                          setSelectedImages([]);
                          setTest_array(demographicSampleImages);
                ...
                <div className="container">
                  ...
                  <div className="mt-3">
                    {(loading ? [...Array(1)] : result?.faces).map((face: any, id: any) => (
                      <div className="d-flex align-items-center me-3 mb-3" key={id}>
                        {" "}
                        {loading ? (
                          <ShimmerThumbnail className="m-0" height={90} width={90} rounded />
                        ) : (
                          <div className="">
                            <img src={`${face.face_url}`} className="img-fluid" width="100" />
                          </div>
                        )}
                        <div className="d-flex justify-content-between flex-column ms-2 gap-2">
                          {loading ? (
                            <ShimmerThumbnail className="mb-0" height={20} width={20} rounded />
                          ) : (
                            <p className="mb-0 font-14 ms-2">
                              Age: <b>{face.age}</b>
                            </p>
                          )}
                          {loading ? (
...
export default demographicDetection;
