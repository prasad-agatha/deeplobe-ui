...
import { poseDetectionSampleImages } from "common_functions/sample_images.ts/poseDetectionSampleImages";
// toasts
...
const poseDetection: FC = () => {
  ...
  const [test_array, setTest_array] = useState(poseDetectionSampleImages);
  ...
  const detectFaces = async () => {
    ...
    if (Object.keys(uploadedFile).length > 0 || imageUrl.length > 0 || selectedImages.length > 0) {
      ...
      if (error) {
        ...
        tempfile = formData;
        pretrainedService
          .poseDetection(tempfile)
          .then((res) => {
            setResult({
              ...result,
              key_url: res.original_img_url,
              skeletal_url: res.skeletal_img_url,
            });
            setLoading(false);
          })
          .catch((err) => {
  ...
  const resetAll = (e) => {
    ...
    setTest_array(poseDetectionSampleImages);
    setSelectedImages([]);
  };
  ...
  const imageClick = (image) => {
    ...
    //Reset Upload images
    setSelectedImages([]);
    let updated_sample = poseDetectionSampleImages.map((item: any) => {
      if (item.id == image.id) {
        return {
          id: image.id,
        ...
        <div className="">
          <div className="row mx-5 my-5">
            ...
            <div className="card border-0 p-3 bg-white">
              <div className="">
                <h4 className="page-title ms-3">Pose detection</h4>
              </div>
            </div>
            ...
            <div className="card border-0 bg-white pb-3">
              <div className="my-2 mx-4 page-content">
                <p className="mb-3">
                  Detects the location of ears, eyes, shoulders, and other major human body parts of
                  people within an image. Test the model by selecting an image from the sample
                  images provided below or, alternatively, you can either upload an image from your
                  computer or provide the URL of any image in the box provided below. Click “Run
                  model”.
                </p>
              </div>
              <SampleCodeAccordion
            ...
            <div className="card border-0 bg-white">
              {submit ? (
                <div className="container">
                  ...
                  <div className="d-flex row justify-content-around mt-3 gap-3 gap-xl-0">
                    <div className="col-12 col-xl-6">
                      <div className="d-flex justify-content-center w-100  bg-light p-3 p-xl-4 mb-4 shimmer-div">
                        {loading ? (
