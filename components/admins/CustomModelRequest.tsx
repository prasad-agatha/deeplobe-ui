...
import CustomModalPopover from "@components/popover/CustomModalPopover";
...
const CustomModelRequest: FC = () => {
  const { data, mutate } = useSWR(`/contact-us/`, async () => {
    const res = await supportService.customModalRequestList();
    return res;
  });
  ...
  const columns: any = [
    ...
    },
    {
      name: "Actions",
      maxWidth: "120px",
      selector: (row: any) => <CustomModalPopover row={row} mutate={mutate} />,
      sortable: true,
      wrap: true,
      center: true,
    },
...
export default CustomModelRequest;
