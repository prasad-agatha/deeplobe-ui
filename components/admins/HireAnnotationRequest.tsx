...
import HireAnnotationPopover from "@components/popover/HireAnnotationPopover";
...
const HireAnnotationRequest = () => {
  const { data, mutate } = useSWR(`/annotation-hire-expert/`, async () => {
    const res = await supportService.annotationHireRequestList();
    return res;
  });
  ...
  const columns: any = [
    ...
    },
    {
      ...
      selector: (row: any) => (
        <OverlayTrigger
          placement="bottom"
          overlay={<Tooltip id="tooltip-engine">{row.subject}</Tooltip>}
        >
    ...
    },
    {
      name: "Actions",
      maxWidth: "120px",
      selector: (row: any) => <HireAnnotationPopover row={row} mutate={mutate} />,
      sortable: true,
      wrap: true,
      center: true,
    },
...
export default HireAnnotationRequest;
