...
const UpdateUserModal = ({ models, row, show, setShow, mutate }) => {
  const options = () =>
    models.map((e) => {
      return {
        id: e.id,
  ...
  useEffect(() => {
    ...
    if (
      ...
      (row.role === "annotator" && row.models[0].select.includes("All"))
    )
      setSelectedOptions([{ label: "All", value: "*" }, ...options()]);
    else setSelectedOptions([...options().filter((e) => row.models[0].select.includes(e.id))]);
    setAccess({
      create_model: row.models[0].create_model || false,
      delete_model: row.models[0].delete_model || false,
      test_model: row.models[0].test_model || false,
      api: row.models[0].api || false,
    });
  }, []);
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      const select = [];
      if (
        (selectedOptions.length > 0 && selectedOptions[0].label === "All") ||
        role === "collaborator"
      )
        select.push("All");
      else
      ...
      console.log(selectedOptions, select, access, "3");
      await subService.updateCollaboratorInvitation(row.collaborator_id, {
        role,
  ...
  return (
    ...
    >
      <Modal.Header style={{ backgroundColor: "white" }}>
        <Modal.Title className="text-primary fw-bold font-inter">Update User</Modal.Title>
      </Modal.Header>
      <Modal.Body className="d-flex flex-column pt-0 p-3 mx-2" style={{ gap: "0.5rem" }}>
          ...
          {["collaborator", "annotator"].map((ele: any, id: any) => (
            ...
            >
              <input
                ...
                readOnly
              />
              <p className="m-0">{ele === "collaborator" ? "Collaborator" : "Annotation Expert"}</p>
            </label>
          ))}
        </div>
        {role !== "collaborator" ? (
              ...
              <div key={id} className="d-inline-flex w-50">
                <label className="d-flex align-items-center cr-p">
                  <input
                    ...
                    checked={access[ele]}
                    onChange={(e: any) => {
                      console.log(access, { ...access, [ele]: !access[ele] });
                      setAccess({ ...access, [ele]: e.target.checked });
                    }}
                  />
                  <p className="m-0">
                    {" "}
        ...
        <div>
          <button className="btn btn-primary px-4 mt-3 mb-2" onClick={updateUser}>
            {loading && (
              <Spinner
                animation="border"
                className="me-2"
