...
const HireAnnotationUpdateModal = ({ updateModal, setUpdateModal, row, mutate }) => {
  const { values, handleSubmit, handleCancel, handleChange }: any = useFormik({
    ...
    onSubmit: (values) => {
      supportService
        .updateHireRequest({ status: values.status.toLowerCase() }, row.id)
        .then(() => {
          mutate();
...
export default HireAnnotationUpdateModal;
