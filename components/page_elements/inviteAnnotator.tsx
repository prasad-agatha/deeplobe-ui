...
const InviteAnnotator = ({ models, annotationInvitation, setAnnotationInvitation }) => {
  const options = () =>
    models.map((e) => {
      return {
        id: e.id,
  ...
  return (
    <>
      <Modal
        show={annotationInvitation}
        className="delete-modal"
        onHide={() => setAnnotationInvitation(false)}
        centered
        backdrop="static"
        keyboard={false}
      >
        <Modal.Header style={{ backgroundColor: "white" }}>
          <Modal.Title className="text-primary fw-bold font-inter">Invite User</Modal.Title>
        </Modal.Header>
        <Modal.Body className="d-flex flex-column pt-0 p-3 mx-2" style={{ gap: "0.5rem" }}>
          ...
          <div>
            ...
            <button
              className="btn btn-sm ms-3 border-0 mt-3 mb-2"
              onClick={() => setAnnotationInvitation(false)}
            >
              Cancel
            </button>
          </div>
...
export default InviteAnnotator;
